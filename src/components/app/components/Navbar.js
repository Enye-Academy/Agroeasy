...
import { PRODUCER_TEXT, LOGO, MARKET_TEXT, NAVBAR, PATHS, USER_AVATAR } from '../constants';

const { Item } = Menu;
const { Header } = Layout;
const { AVATAR, SHAPE, SIZE, SOURCE } = LOGO;
const { MAIN_NAV, NAV_MENU, NAV_MODE, NAV_THEME } = NAVBAR;
const { PRODUCER_ITEMS, HOME, MARKET, PROFILE, SIGN_IN, SIGN_UP } = PATHS;
...
const items = [
    ...
    <AppLink key={PRODUCER_ITEMS} to={PRODUCER_ITEMS}>{PRODUCER_TEXT}</AppLink>,
    <Signin key={SIGN_IN} />,
    <Signup key={SIGN_UP} />,
];
